...
router.post('/login', bp, (req, res) => {
    mysql.query('select username,password,token,sex,phone,nickname,qq,avatar from hs_user where username=? and password=?', [req.body.username, MD5(req.body.password)], (err, result) => {
        if (err) throw err;
        if (result.length > 0) {
            res.send({
                data: result,
